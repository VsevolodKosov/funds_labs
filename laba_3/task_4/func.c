...
status_code string_to_uint(const char *str, unsigned long *result)
{
    char *endinp;
...
status_code post_init(Post *post)
{
...
status_code get_address(Address *address)
{
    if (!address)
        return MemoryError;
...
status_code get_weight(double *weight)
{
    char _weight[128];
    if (!weight)
        return MemoryError;
...
status_code get_postal_index(String *str)
{
...
status_code validate_date(const char *date)
{
...
status_code validate_time(const char *time)
{
...
status_code get_time(String *str)
{
...
status_code add_mail(Post *post, int *count_mails)
{
    Mail *mail_realloc;
    status_code state;
...
status_code delete_mail(Post *post, int *count_mails)
{
...
status_code search_mail(Post *post, int *count_mails)
{
    if (!post || !post->mail_arr)
        return MemoryError;
...
status_code set_delivered(Post *post, int *count_mails)
{
    if (!post || !post->mail_arr)
        return MemoryError;
...
status_code search_delivered(Post *post, int count_mails, int *count_delivered, Mail **arr_delivered)
{
...
status_code search_overdue(Post *post, int count_mails, int *count_overdue, Mail **arr_overdue)
{
